...
bool hotspot_at(struct mako_hotspot *hotspot, int32_t x, int32_t y) {
  return x >= hotspot->x && y >= hotspot->y &&
         x < hotspot->x + hotspot->width && y < hotspot->y + hotspot->height;
}
...
struct mako_notification *create_notification(struct mako_state *state) {
  struct mako_notification *notif = calloc(1, sizeof(struct mako_notification));
  if (notif == NULL) {
...
void close_notification(struct mako_notification *notif,
  ...
  if (!notif->style.history || notif->state->config.max_history <= 0) {
...
size_t format_text(const char *format, char *buf,
                   mako_format_func_t format_func, void *data) {
  size_t len = 0;
...
static void try_invoke_action(struct mako_notification *notif,
  ...
  wl_list_for_each(action, &notif->actions, link) {
    if (strcmp(action->key, target_action) == 0) {
      ...
      if (ctx != NULL) {
        activation_token =
            create_xdg_activation_token(ctx->surface, ctx->seat, ctx->serial);
      }
...
void notification_handle_button(struct mako_notification *notif,
                                uint32_t button,
...
get_last_notif_by_urgency(struct wl_list *notifications,
...
void insert_notification(struct mako_state *state,
                         struct mako_notification *notif) {
...
// with criteria.
int group_notifications(struct mako_state *state,
  ...
  // anymore.
  wl_list_for_each(notif, &matches, link) { notif->group_count = count; }
